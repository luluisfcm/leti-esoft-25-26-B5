...
Added a nurse console menu option to launch RecordVaccinationView; on success, confirms the assignment to recovery.
Demonstrated end-to-end: select checked-in SNS user, choose vaccine/lot, confirm, observe appointment status transitions to InRecovery.
Verified REST with curl/Postman: valid POST /api/vaccinations returns 201 Created; invalid empty lot is rejected.
