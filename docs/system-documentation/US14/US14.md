...
|:--- |:--- |:--- |:--- |
| 1 | ...handling the user's action to start registration? | `RegisterEmployeeUI` | **Controller (GRASP)**. The UI layer is the first to receive input from the Actor. |
| 2 | ...displaying the form fields to be filled? | `RegisterEmployeeUI` | The UI class is responsible for rendering the view and requesting the data. |
| 3 | ...capturing the data typed by the user (name, address, etc.)? | `RegisterEmployeeUI` | The UI collects the data from the form fields. |
| 4 | ...displaying the list of available roles for selection? | `RegisterEmployeeUI` | The UI presents the options (dropdown, list, etc.) to the user. |
| 5 | ...capturing the selected role and delegating the creation request? | `RegisterEmployeeUI` | **Controller (GRASP)**. The UI bundles all data and calls the application Controller. |
| 6 | ...coordinating the *creation* (in memory) of the object for confirmation? | `RegisterEmployeeController` | **Controller (GRASP)**. Orchestrates the creation logic (by calling the Store). |
| 7 | ...coordinating the *persistence* (saving) of the object upon confirmation? | `RegisterEmployeeController` | **Controller (GRASP)**. Orchestrates the saving logic and uniqueness validation (AC14-4). |
| 8 | ...displaying the final success message to the user? | `RegisterEmployeeUI` | The UI is responsible for giving final feedback to the Actor. |
