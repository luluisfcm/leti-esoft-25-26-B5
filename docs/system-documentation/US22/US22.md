...
**Endpoint Exposure (CREATE):** A POST-type route was implemented to register the arrival (check-in) of an SNS user for a given vaccination center, receiving the SNS user number and the vaccination center identifier in the request (JSON body). Optionally, an additional confirmation route is used to finalize the check-in after previewing the scheduled appointment for the current day.

**Flow Management in the Controller:** The RegisterArrivalController acts as the entry point, responsible for validating authentication, extracting request data, coordinating the execution of the use case, and returning an HttpResult consistent with the generic HTTP flow.

**Business Rules and Validation:** The ArrivalService reuses domain logic to (i) locate the SNS user’s appointment scheduled for the current day at the selected vaccination center, and (ii) enforce AC22-1 by preventing duplicate check-ins for the same appointment. When no appointment exists for the day, the service returns a failure result, as required.

**Persistence and State Update:** The system interacts with AppointmentRepository, CheckInRepository, and WaitingRoomRepository to create and persist the CheckIn, assign the default waiting room of the vaccination center, and update the Appointment status to Arrived while storing the arrival timestamp.

**Data Transformation (Response Mapping):** To keep the REST contract independent from the domain model, the operation returns only the necessary information to the client (e.g., check-in identifier, assigned waiting room name, and arrival date-time) encoded as JSON within the HttpResult.
