...
|:--------------:|:-------------------------------------------|:-------|:------------------------------|
| Step 1 | interacting with the actor (SNS User)? | ScheduleVaccineView | **Pure Fabrication** for UI interaction; isolates UI concerns from domain logic. |
|  | receiving the system operation to start scheduling? | ScheduleVaccineController | **GRASP Controller**: first object to receive the system operation for this use case. |
| Step 2 | requesting the vaccine type from the SNS user? | ScheduleVaccineView | **Pure Fabrication**: responsible for prompting and collecting typed/selected input. |
| Step 3 | obtaining the list of available vaccine types to present? | VaccineTypeRepository | **Information Expert**: stores and retrieves `VaccineType` entities. |
| Step 4 | presenting available vaccination centers for the selected vaccine type? | VaccinationCenterRepository | **Information Expert**: holds the data required to obtain centers that support a given `VaccineType`. |
| Step 5 | obtaining available time slots for the selected center and vaccine type? | ScheduleService | **Information Expert**: encapsulates availability rules and slot computation logic. |
| Step 6 | collecting the scheduling data (vaccine type, center, slot, contact info)? | ScheduleVaccineRequestDTO | **Parameter Object / Pure Fabrication**: aggregates user input into a single data carrier for the application layer. |
| Step 7 | preparing and returning the appointment preview data shown to the SNS user? | ScheduleService | **Information Expert**: has access to all required data to assemble the preview information. |
| Step 8 | validating constraints at confirmation time (e.g., slot still available, no conflicting future appointment) | ScheduleService | **Information Expert** for business rules: centralizes validation and applies acceptance criteria before creation. |
|  | creating the appointment after confirmation | ScheduleService | **Creator**: receives all necessary data to instantiate a new appointment. |
|  | persisting the created appointment | VaccineAppointmentRepository | **Pure Fabrication**: encapsulates storage and retrieval of appointment entities. |
| Step 9 | informing the SNS user of success or failure | ScheduleVaccineView | **Pure Fabrication**: provides user feedback while keeping decision logic in the service layer. |
