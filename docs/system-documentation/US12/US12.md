...
- Access to the vaccine repository (via `RepositoryFactory` → `VaccineRepository`) is required to fetch the data set.
...
- Sorting by brand and grouping by type is a business rule derived from AC12-1 and is implemented in the application/service layer, not explicitly represented in the Domain Model.
...
|:--------------:|:---------------------------------------------------------------------------|:-----------------------------|:----------------------------------------------------------------------------------------------|
| Step 1         | interacting with the actor (Administrator)?                               | ListVaccinesView             | **Pure Fabrication / Information Expert** for user interaction.                               |
|                | receiving the system operation to list all vaccines?                      | ListVaccinesController       | **GRASP Controller** coordinating the use case.                                               |
|                | coordinating the execution of the US?                                     | ListVaccinesController       | **Controller** centralizes the workflow.                                                      |
|                | obtaining the collection of vaccines from persistence?                    | VaccineRepository            | **Information Expert / Pure Fabrication** that manages stored vaccines.                       |
|                | grouping vaccines by type and sorting by brand (AC12-1)?                  | VaccineService               | **Information Expert** for the specific use-case rule, encapsulating the grouping and sort.   |
...
|                | informing the (in)success of the listing to the actor?                    | ListVaccinesView             | **Information Expert** for user feedback/presentation.                                        |

The `Administrator` uses a `RepositoryFactory` to obtain the concrete `VaccineRepository` and adopt a `VaccineService`, which applies the grouping and ordering rule defined in AC12-1.

### Systematization

According to the rationale and the SSD/SD, the following conceptual classes are promoted to software classes:

- Vaccine
- VaccineType

Other software classes (Pure Fabrication / application layer) identified:

- ListVaccinesView
- ListVaccinesController
- VaccineService
- VaccineRepository (accessed via RepositoryFactory)
