...
| :------------- | :------------------------------------------- | :------------------------------------ | :---------------------------------------------------------------------------- |
| 1              | ...starting the center registration process? | `RegisterVaccinationCenterUI`         | **Low Coupling & High Cohesion** – The UI only handles user interaction.      |
| 2              | ...displaying the form fields?               | `RegisterVaccinationCenterUI`         | **High Cohesion** – Field presentation belongs to the UI layer.               |
| 3              | ...showing available vaccine types?          | `RegisterVaccinationCenterUI`         | **High Cohesion** – UI is responsible for selection lists.                    |
| 4              | ...capturing the entered data?               | `RegisterVaccinationCenterUI`         | **Information Expert** – The UI has direct access to user-entered data.       |
| 5              | ...coordinating the creation of the center?  | `RegisterVaccinationCenterController` | **Controller Pattern** – Coordinates the system operation.                    |
| 6              | ...validating business rules?                | `VaccinationCenterService`            | **Information Expert & High Cohesion** – Business rules belong to the domain. |
| 7              | ...persisting the center?                    | `VaccinationCenterRepository`         | **Pure Fabrication** – Responsible for persistence.                           |
| 8              | ...informing the success of the operation?   | `RegisterVaccinationCenterUI`         | **High Cohesion** – User feedback belongs to the UI.                          |
